...
3. 对每一行进行翻译。配合视频窗口和以下快捷键提高效率：
   ...
   - `Alt-J`智能合并对话（空格分隔）：把两行的中文和日文部分分别合并在一起，并用空格隔开；
   - `Alt-Shift-J`智能合并对话（无分隔）：把两行的中文和日文部分分别合并在一起；
...
上述的“中文和日文部分分别合并在一起”等功能属于我组编写的自动化脚本，详细介绍见[lua_scripts](https://github.com/zhimengsub/lua_scripts)，翻译君主要关注**智能合并对话**、**添加对白特效**、**删除对白特效**、**检查中日符号一致性**等功能即可。

使用方法：

1. 手动在`自动化`菜单中选择使用：

   ![image-20240615195028419](README.src/Automation.png)

2. 使用快捷键触发，如`Alt-J`触发`智能合并对话（空格分隔）`。

   任意一个脚本都可以设置对应的快捷键，设置方法见下文。其中Command栏只需输入`automation/`，会自动弹出可以设置的指令，按需要选择即可。
